import useFetch from '../../hooks/useFetch';
import './Popular.css';

export default function Popular() {

    const { data, loading, error } = useFetch("/hotels?featured=true&limit=4");

    const images = [
        "https://cf.bstatic.com/xdata/images/hotel/square600/13125860.webp?k=e148feeb802ac3d28d1391dad9e4cf1e12d9231f897d0b53ca067bde8a9d3355&o=&s=1",
        "https://cf.bstatic.com/xdata/images/hotel/max1280x900/215955381.jpg?k=ff739d1d9e0c8e233f78ee3ced82743ef0355e925df8db7135d83b55a00ca07a&o=&hp=1",
        "https://cf.bstatic.com/xdata/images/hotel/square600/13125860.webp?k=e148feeb802ac3d28d1391dad9e4cf1e12d9231f897d0b53ca067bde8a9d3355&o=&s=1",
        "https://cf.bstatic.com/xdata/images/hotel/max1280x900/322658536.jpg?k=3fffe63a365fd0ccdc59210188e55188cdb7448b9ec1ddb71b0843172138ec07&o=&hp=1"
    ];

    return (
        <div className="popular">
            {loading ? "Loading..." :
            <>
                {data.map((hotel, i) => (
                    <div className="popular-item" key={hotel._id}>
                        <img 
                            src={hotel.photos[0]}
                            alt="" 
                            className="popular-img" 
                        />
                        <span className="popular-name">{hotel.name}</span>
                        <span className="popular-city">{hotel.city}</span>
                        <span className="popular-price">Starting from {hotel.cheapestPrice}</span>
                        {hotel.rating && 
                        <div className="popular-rating">
                            <button>{hotel.rating}</button>
                            <span>Excellent</span>
                        </div>}
                    </div>))}
            </>}
        </div>
    );
}